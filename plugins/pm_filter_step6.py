...
async def advantage_spell_chok(client, message):
    ...
    try:
        ...
        # Severe typos can make IMDb bulk search return nothing. Reuse the existing
        # spelling-correction engine first, then search IMDb with the corrected title.
        if not movies:
    ...
    if not suggestions:
        corrected = await ai_spell_check(chat_id, search)
        if corrected and normalize(corrected) != normalize(search):
            try:
                corrected_movies = await get_poster(corrected, bulk=True)
                corrected_ranked = []
                seen_corrected = set()
                for movie in corrected_movies or []:
                    title = movie.get('title')
                    if not title:
                        continue
                    key = normalize(title)
                    if not key or key in seen_corrected:
                        continue
                    seen_corrected.add(key)
                    score = process.extractOne(normalize(corrected), [key])[1]
                    kind = str(movie.get('kind') or '').lower()
                    if kind in ('movie', 'tv series', 'tv mini series', 'tv movie'):
                        score += 3
                    corrected_ranked.append((score, movie))
                corrected_ranked.sort(key=lambda item: item[0], reverse=True)
                suggestions = [movie for score, movie in corrected_ranked if score >= 55][:6]
            except Exception:
                suggestions = []
    ...
    for movie in suggestions:
        ...
        buttons.append([
            InlineKeyboardButton(
                text=label,
                callback_data=f"spol#{getattr(movie, 'movieID', None) or (movie.get('movieID') if hasattr(movie, 'get') else None)}#{user}"
            )
        ])
